...
텍스트 아티팩트 기능과 기존 텍스트 테스트는 이미지 안전성, 시각적 가독성, 모델 정확도, 지연 또는
비용 절감을 입증하지 않는다.
...
남긴다. 현재 저장소의 구현은 UTF-8 텍스트 인덱싱, 해시, 라인 맵, 손실 요약 차단을 제공하지만 이미지
변환기, OCR 결합기, 호스팅 실행기 또는 사용량 계측기는 제공하지 않는다.
...
제시하지 않는다. 현재의 텍스트 엔지니어링 테스트는 텍스트 저장소의 계약을 검증할 뿐 이미지 방법의
효과를 검증하지 않는다.
...
재현 가능한 관측을 남기는지다. 현재 방법은 설계 단계이고 runnable image implementation이나 측정 결과가
없다. 다음 단계는 이미지 ingest와 경로 안전성, resize/crop provenance, guard와 fallback, 평가 adapter를
